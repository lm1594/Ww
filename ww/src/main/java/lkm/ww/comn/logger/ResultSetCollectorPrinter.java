...
public class ResultSetCollectorPrinter {
	 ...
	 *
	 */
	public String getResultSetToPrint(ResultSetCollector resultSetCollector) throws UnsupportedEncodingException {
		...
		for (int column = 1; column <= columnCount; column++) {
			maxLength[column - 1] = resultSetCollector.getColumnName(column).getBytes("EUC-KR").length;
		}
		if (resultSetCollector.getRows() != null) {
			for (List<Object> printRow : resultSetCollector.getRows()) {
				...
				for (Object v : printRow) {
					if (v != null) {
						int length = v.toString().getBytes("EUC-KR").length;
	 ...
	 * @throws UnsupportedEncodingException
	 */
	private static String padRight(String s, int n) throws UnsupportedEncodingException {
		return StringUtils.rightPad(s, s.length() + n - s.getBytes("EUC-KR").length);
